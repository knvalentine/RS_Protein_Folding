...
class PatternAnalyzer:
    ...
    def _calculate_p_geom(self, n_loops: int, contact_order: float,
        ...
        # RS logic forbids empirical clipping; ensure numerical floor only
        # Probability-like factors must not exceed 1.0 by definition
        p_geom = min(p_geom, 1.0)
    ...
    def _count_beta_registries(self, graph: nx.Graph, voxel_map: Dict,
        ...
        # If no registries found but we have sheet content, estimate from sequence
        from math import pow
        sheet_frac_threshold = PHI ** -2  # ≈ 0.382, RS-derived (phi^{-2})
        if total_registries == 0 and self._analyze_secondary_structure(torsion_states)[1] > sheet_frac_threshold:
