...
def derive_mobility_constant():
    ...
    # RS-DERIVED DIRECTIONAL EFFICIENCY
    # In RS, directional bias comes from phase gradient alignment
    # For unit information pressure gradient:
    # - Phase difference across voxel = 2π/φ (golden angle)
    # - Probability of forward vs backward = |cos(Δφ/2)|²
    # - Net directional bias = P(forward) - P(backward)
    
    phase_gradient = 2 * np.pi / PHI  # Golden angle
    p_forward = np.cos(phase_gradient/4)**2  # Half the gradient for forward
    p_backward = np.cos(-phase_gradient/4)**2  # Opposite for backward
    directional_efficiency_rs = p_forward - p_backward
    
    # For small angles: cos²(θ/4) ≈ 1 - θ²/32
    # Net bias ≈ θ²/16 = (2π/φ)²/16 ≈ 0.096
    
    print(f"RS-derived directional efficiency: {directional_efficiency_rs:.3f}")
    print(f"Approximation: {(phase_gradient**2)/16:.3f}")
    
    # Use RS-derived value
    displacement_rate = recoil_velocity_A_per_ps * effective_rate_per_ps * directional_efficiency_rs
...
def derive_mobility_from_cumulative_recoil():
    ...
    # Use RS-derived directional efficiency
    phase_gradient = 2 * np.pi / PHI  # Golden angle
    directional_factor = (phase_gradient**2) / 16  # ≈ 0.096
    print(f"Using RS-derived directional factor: {directional_factor:.3f}")
